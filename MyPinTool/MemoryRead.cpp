...
static std::tr1::unordered_map<VOID *, uint32_t> visable_addrs;
...
VOID RecordMemRead(VOID * ip, VOID * mem_addr, USIZE mem_size)
{
    ...
    if (mem_addr < stack_border){
        ...
        //std::tr1::unordered_map<MemObj>::iterator iter = visable_addrs.find(mem_obj);
        if (visable_addrs.find(mem_addr) != visable_addrs.end()) {
            if (visable_addrs[mem_addr] < mem_size){
                visable_addrs[mem_addr] = mem_size;
            }
            else{
                return;
            }
...
VOID RecordMemWrite(VOID * ip, VOID * mem_addr, USIZE mem_size)
{
    ...
    if (mem_addr < stack_border){
        if (mem_addr < min_write_mem){
            min_write_mem = mem_addr;
            size_min = mem_size;
        }
        if (mem_addr > max_write_mem){
            max_write_mem = mem_addr;
            size_max = mem_size;
        }
    }
}
