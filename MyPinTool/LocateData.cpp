...
uint64_t data_seg_start = 0;
uint64_t data_seg_end = 0;
...
KNOB<uint64_t>   KnobDataStart(KNOB_MODE_WRITEONCE,  "pintool",
    "data_start", "", "no description");
    
KNOB<uint64_t>   KnobDataEnd(KNOB_MODE_WRITEONCE,  "pintool",
    "data_end", "", "no description");
...
VOID RecordMemRead(VOID * ip, VOID * mem_addr, USIZE mem_size)
{
    ...
    if (data_seg_start <= (uint64_t)mem_addr and (uint64_t)mem_addr <= data_seg_end){
    ...
    fprintf(trace, "%p:%p,%lu", ip, mem_addr, mem_size);
    if (mem_size == 8){
        fprintf(trace," : 0x%lx\n", *(uint64_t *)(mem_addr));
    }
    else{
