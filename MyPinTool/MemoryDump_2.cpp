...
KNOB<int32_t>   KnobDataIndex(KNOB_MODE_WRITEONCE,  "pintool",
    "data_index", "1", "no description");
...
VOID Dump(VOID * ip, ADDRINT rdi_value){
    //fprintf(trace, "Start:\n");
    // get address from register rdi 
    printf("rdi: %p\n", (void *)rdi_value);
    //void ** param_start = ((void **)rdi_value+1);
    void ** param_start = ((void **)rdi_value+data_index);
    printf("rdi: %p\n", *param_start);
    printf("rdi: %p\n", *((void **)*param_start));
    param_start = (void **)*param_start;
    printf("%p\n", *((void **)*param_start));
    PrintDword((const int *)*param_start, (int)length);
}
...
int main(int argc, char *argv[])
{
    ...
    data_index = KnobDataIndex.Value();

    // debug
    printf("output: %s, dump_addr: %p, dump_length: %lu, dump_point: %p, data_index: %d\n", fileName.c_str(), (void *)dump_addr, length, (void *)dump_point, data_index);
    //return 0;
