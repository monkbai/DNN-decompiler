...
class NET(nn.Module):
    def __init__(self, seq_len):
        ...
        set_weights(net[-1], '0020.tvmgen_default_fused_less_add_where_take_transpose_reshape_nn_pad_compute_.params_0.json')

        net.append(nn.AvgPool2d((seq_len, 1)))

        net.append(nn.Linear(in_features=100, out_features=1))
        set_weights(net[-1], '0024.tvmgen_default_fused_nn_contrib_dense_pack_compute_.params_0.json')
        set_biases(net[-1], '0026.tvmgen_default_fused_reshape_add_compute_.params_0.json')
...
if __name__ == "__main__":
    ...
    torch.onnx.export(vgg,               # model being run
                      ...
                      "embedding_2_tvmO3_rebuild.onnx",   # where to save the model (can be a file or file-like object)
