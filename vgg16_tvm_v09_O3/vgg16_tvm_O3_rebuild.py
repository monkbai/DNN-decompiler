...
class SE_VGG(nn.Module):
    def __init__(self, num_classes):
        ...
        set_biases(net[0], './0049.biases_0.json')
        ...
        set_weights(net[2], './0053.weights_0.json')
        set_biases(net[2], './0053.biases_0.json')
        ...
        set_weights(net[5], './0057.weights_0.json')
        set_biases(net[5], './0057.biases_0.json')
        net.append(nn.ReLU())  # 6
        net.append(nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1))  # 7
        set_weights(net[7], './0061.weights_0.json')
        set_biases(net[7], './0061.biases_0.json')
        ...
        set_weights(net[10], './0065.weights_0.json')
        set_biases(net[10], './0065.biases_0.json')
        net.append(nn.ReLU())  # 11
        net.append(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, padding=1, stride=1))  # 12
        set_weights(net[12], './0069.weights_0.json')
        set_biases(net[12], './0069.biases_0.json')
        net.append(nn.ReLU())  # 13
        net.append(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, padding=1, stride=1))  # 14
        set_weights(net[14], './0069.weights_1.json')
        set_biases(net[14], './0069.biases_1.json')
        ...
        set_weights(net[17], './0073.weights_0.json')
        set_biases(net[17], './0073.biases_0.json')
        net.append(nn.ReLU())  # 18
        net.append(nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=1))  # 19
        set_weights(net[19], './0077.weights_0.json')
        set_biases(net[19], './0077.biases_0.json')
        net.append(nn.ReLU())  # 20
        net.append(nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=1))  # 21
        set_weights(net[21], './0077.weights_1.json')
        set_biases(net[21], './0077.biases_1.json')
        ...
        set_weights(net[24], './0081.weights_0.json')
        set_biases(net[24], './0081.biases_0.json')
        net.append(nn.ReLU())  # 25
        net.append(nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=1))  # 26
        set_weights(net[26], './0081.weights_1.json')
        set_biases(net[26], './0081.biases_1.json')
        net.append(nn.ReLU())  # 27
        net.append(nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=1))  # 28
        set_weights(net[28], './0081.weights_2.json')
        set_biases(net[28], './0081.biases_2.json')
        ...
        set_weights(classifier[-1], './0087.dense_weights_0.json')
        set_biases(classifier[-1], './0087.biases_0.json')
        #print(classifier[0].bias.shape)
        #print(classifier[0].bias)
        classifier.append(nn.ReLU())
        # classifier.append(nn.Dropout(p=0.5))
        classifier.append(nn.Linear(in_features=4096, out_features=4096))
        set_weights(classifier[-1], './0090.dense_weights_0.json')
        set_biases(classifier[-1], './0090.biases_0.json')
        classifier.append(nn.ReLU())
        # classifier.append(nn.Dropout(p=0.5))
        classifier.append(nn.Linear(in_features=4096, out_features=self.num_classes))
        set_weights(classifier[-1], './0084.dense_weights_0.json')
        set_biases(classifier[-1], './0084.biases_0.json')
...
if __name__ == "__main__":
    ...
    with open("/export/d1/zliudc/DLE_Decompiler/TVM/rebuild_ida/TVM-v0.9.dev/vgg16_tvm_O3/cat.bin", 'br') as f:
        bin_data = f.read()
